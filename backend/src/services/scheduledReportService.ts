...
export const scheduledReportService = {
  async list(params: { page: number; perPage: number; enabled?: boolean }) {
    ...
    let q = supabase.from('scheduled_reports').select('*', { count: 'exact' }).order('created_at', { ascending: false });
  ...
  async getById(id: string) {
    const { data, error } = await supabase.from('scheduled_reports').select('*').eq('id', id).maybeSingle();
    if (error) throw error;
  ...
  async create(input: ScheduledReportInput) {
    const { data, error } = await supabase.from('scheduled_reports').insert(toDb(input)).select('*').single();
    if (error) throw error;
    return data;
  },
